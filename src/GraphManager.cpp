...
void GraphNode::AddNeighbor(GraphEdge node)
{
    m_Neighbors.push_back(node);
}

void GraphNode::AddNeighbor(GraphNode* node, double distance)
{
    GraphEdge Edge;
    Edge.Cost = distance;
...
double GraphNode::DistanceTo(GraphNode* Node)
{
...
Vector2u GraphNode::GetPosition() const
{
    return m_Position;
}

GraphNode::GraphNode(Vector2u Pos)
{
    m_Position = Pos;
}


std::vector<GraphNode*> GraphManager::FindPath(GraphNode* Start, GraphNode* End) const
{
...
GraphManager& GraphManager::CreateGraph(const uint NumNodes, const Vector2u Field)
{
...
GraphManager& GraphManager::FormEdges()
{
...
PathCallback GraphManager::GetPathCallback()
{
...
uint GraphManager::NumNodes()
{
    return static_cast<uint>(m_GraphList.size());
}

uint GraphManager::NumNodesNotTaken()
{
    return NumNodes() - NumTaken;
}

GraphNode* GraphManager::TakeNode()
{
...
GraphNode* GraphManager::operator[](uint index) const
{
